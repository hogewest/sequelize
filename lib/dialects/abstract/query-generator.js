...
const QueryGenerator = {
  ...
  */
  ...
  generateJoin(include, topLevelInfo) {
    ...
    if (association.extraForeignKeys) {
      const extraForeignKeysJoinConditions = association.extraForeignKeys.map(key => {
        const source = left.rawAttributes[key.source].field;
        const target = right.rawAttributes[key.target].field;
        const extraForeignKeyConditionLeft = `${this.quoteTable(asLeft)}.${this.quoteIdentifier(source)}`;
        const extraForeignKeyConditionRight = `${this.quoteTable(asRight)}.${this.quoteIdentifier(target)}`;
        return `${extraForeignKeyConditionLeft} = ${extraForeignKeyConditionRight}`;
      }).join(' AND ');
      
      joinOn = `(${joinOn} AND ${extraForeignKeysJoinConditions})`;
    }
